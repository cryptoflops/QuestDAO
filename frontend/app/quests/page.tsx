...
export default function QuestsPage() {
    ...
    const userAddress = userSession.isUserSignedIn()
        ? userSession.loadUserData().profile.stxAddress[IS_MAINNET ? 'mainnet' : 'testnet']
        : null;
        ...
        const fetchQuests = async () => {
            ...
            try {
                ...
                for (let i = 1; i <= count; i++) {
                    ...
                    // 3. Fetch Completion Status if user signed in
    ...
    // editorial minimalism: asymmetric grid pattern
    const getGridSpan = (index: number) => {
        const pattern = [
            'md:col-span-8', // 1. Hero item
            'md:col-span-4', // 2. Sidebar item
            'md:col-span-6', // 3. Balanced split
            'md:col-span-6', // 4. Balanced split
            'md:col-span-12' // 5. Full width footer item
        ];
        return pattern[index % pattern.length];
    };

    return (
        <main className="min-h-screen bg-transparent text-stacks-black">
